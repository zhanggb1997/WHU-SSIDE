...
class BasicDispEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.args = args
        geo_planes = (2*args.MODEL.SET.IGEV.CORR_RADIUS + 1) * 2 + 96
...
class BasicMultiUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dims=[]):
        super().__init__()
        self.args = args
        self.geo_encoder0 = GeoEncoder(geo_planes=2 * (2*args.MODEL.SET.IGEV.CORR_RADIUS + 1) * 8)
        self.geo_encoder1 = GeoEncoder(geo_planes=(2*args.MODEL.SET.IGEV.CORR_RADIUS + 1) * 8)
        self.geo_encoder2 = GeoEncoder(geo_planes=(2*args.MODEL.SET.IGEV.CORR_RADIUS + 1) * 8)
        self.encoder = BasicDispEncoder(args)
        encoder_output_dim = 128

        self.gru04 = ConvGRU(hidden_dims[2], encoder_output_dim + hidden_dims[1] * (args.MODEL.SET.IGEV.N_GRU_LAYERS > 1))
        self.gru08 = ConvGRU(hidden_dims[1], hidden_dims[0] * (args.MODEL.SET.IGEV.N_GRU_LAYERS == 3) + hidden_dims[2])
        self.gru16 = ConvGRU(hidden_dims[0], hidden_dims[1])
        self.disp_head = DispHead(hidden_dims[2], hidden_dim=256, output_dim=1)
        factor = 2**self.args.MODEL.SET.IGEV.N_DOWNSAMPLE
    ...
    def forward(self, net, inp, geo_feat0=None, geo_feat1=None, geo_feat2=None, init_corr=None, selective_weights=None, disp=None, iter04=True, iter08=True, iter16=True, update=True):
        ...
        if iter08:
            if self.args.MODEL.SET.IGEV.N_GRU_LAYERS > 2:
        ...
        if iter04:
            ...
            if self.args.MODEL.SET.IGEV.N_GRU_LAYERS > 1:
