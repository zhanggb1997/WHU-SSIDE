...
def build_gwc_volume(refimg_fea, targetimg_fea, mindisp, maxdisp, num_groups):
    ...
    volume = refimg_fea.new_zeros([B, num_groups, int(maxdisp-mindisp), H, W])
...
def disparity_regression(prob, mindisp, maxdisp, interval):
    assert len(prob.shape) == 4
    disp_values = torch.arange(mindisp, maxdisp, interval, dtype=prob.dtype, device=prob.device)
    disp_values = disp_values.view(1, (maxdisp-mindisp)//interval, 1, 1)
