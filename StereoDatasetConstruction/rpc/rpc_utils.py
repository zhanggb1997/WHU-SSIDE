...
def altitude_range(rpc, x, y, w, h, margin_top=0, margin_bottom=0):
    ...
    # find bounding box on the ellipsoid (in geodesic coordinates)  在椭球体上查找边界框（在测地线坐标中）
    lon_m, lon_M, lat_m, lat_M = geodesic_bounding_box(rpc, x, y, w, h)

    # compute heights on this bounding box  根据外边界框计算高程
    if cfg['exogenous_dem'] is not None:  # 根据外dem计算高程
        h_m, h_M = min_max_heights_from_bbx(cfg['exogenous_dem'],
                                            lon_m, lon_M, lat_m, lat_M, rpc)
        h_m += margin_bottom
        h_M += margin_top
    elif cfg['use_srtm']:  # 根据srtm计算
...
def utm_zone(rpc, x, y, w, h):
    ...
    # determine lat lon of the center of the roi, assuming median altitude 假设高度中值，确定roi中心的经度和纬度
    lon, lat = rpc.localization(x + .5*w, y + .5*h, rpc.alt_offset)[:2]

    return geographiclib.compute_utm_zone(lon, lat)  # 根据经纬度计算utm分带
...
def generate_point_mesh(col_range, row_range, alt_range):
    ...
    by alt_range.
    生成位于col和row range定义的栅格上的三维点的图像坐标（col、row、alt），这些点位于alt range定义的均匀采样高度。
...
def corresponding_roi(rpc1, rpc2, x, y, w, h):
    ...
    specified ROI of im1.  使用rpc确定img的对应roi范围
    ...
    m, M = altitude_range(rpc1, x, y, w, h, 0, 0)  # 计算高程范围

    # build an array with vertices of the 3D ROI, obtained as {2D ROI} x [m, M]  设定3D-ROI范围，立方体八个角点
    a = np.array([x, x,   x,   x, x+w, x+w, x+w, x+w])
    b = np.array([y, y, y+h, y+h,   y,   y, y+h, y+h])
    c = np.array([m, M,   m,   M,   m,   M,   m,   M])

    # corresponding points in im2  输入xyz计算img2中相应的点图像坐标
    xx, yy = find_corresponding_point(rpc1, rpc2, a, b, c)[0:2]

    # return coordinates of the bounding box in im2  图像2中对应边界坐标，获取最小xy和wh
    out = common.bounding_box2D(np.vstack([xx, yy]).T)
    return np.round(out)  # 整数
...
def matches_from_rpc(rpc1, rpc2, x, y, w, h, n):
    ...
    """
    m, M = altitude_range(rpc1, x, y, w, h, 100, -100)  # 根据边缘上限下限 计算最高最低海拔
    lon, lat, alt = ground_control_points(rpc1, x, y, w, h, m, M, n)  # 计算与RPC数据相对应的n个地面控制点（GCP） 地理坐标系。
    x1, y1 = rpc1.projection(lon, lat, alt)  # img1像方位置
    x2, y2 = rpc2.projection(lon, lat, alt)  # img2像方位置
